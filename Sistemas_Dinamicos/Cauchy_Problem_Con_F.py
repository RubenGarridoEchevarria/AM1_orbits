...
def Cauchy_problem_2(t, U0, Temporal_integrator, Diferntial_operator, q, Tolerance):  #Defino el problema de Cauchy Con la F, la condición inicial y se introducé el integrador temporal que se usa
    ...
    for i in range(0,N):          #Crea el vector del problema de Cauchy para luego en cada paso del bucle resolverlo
            ...
            U[i+1,:] = Temporal_integrator(U[i, :] , t[i+1] - t[i], t[i],  Diferntial_operator, q, Tolerance) #Crea el vector problema de Cauchy en el siguiente paso de tiempo y llama al integrador temporal para que lo resulva
